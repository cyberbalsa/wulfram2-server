...
# engine_thunks.cpp is the lone exception: it is x86-only `__declspec(naked)` +
# inline `__asm` register thunks (engine functions whose calling convention no MSVC
# keyword can express). clang-tidy drives clang with the HOST x86_64 triple (the
# MSVC compile_commands has no -m32 it can infer), and clang rejects the x86-only
# `naked` attribute outright. The file holds no analyzable C++ (pure register glue),
# and cppcheck below still scans it, so it is excluded from clang-tidy only.
$Sources = Get-ChildItem -Path (Join-Path $RepoRoot "src") -Recurse -Filter *.cpp |
    Select-Object -ExpandProperty FullName |
    Where-Object { $_ -notmatch 'engine_thunks\.cpp$' }
$TidyFailed = $false
